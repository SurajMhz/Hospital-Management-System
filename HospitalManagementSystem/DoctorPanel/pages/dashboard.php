...
if (isset($_POST['add'])) {
    ...
    $stmt->bind_param("isisssss" . "s", $doctor_id, $patient_name, $age, $gender, $phone, $date, $department, $reason, $source);
    $stmt->execute();
}
...
if (isset($_POST['status'])) {
    ...
    $stmt2 = $conn->prepare("UPDATE appointments SET status = ? WHERE id = ? AND doctor_id = ?");
    $stmt2->bind_param("sii", $new_status, $id, $doctor_id);
    $stmt2->execute();
}

// GET ALL APPOINTMENTS
$appointments = $conn->query("SELECT * FROM appointments WHERE doctor_id=$doctor_id ORDER BY created_at DESC");
...
<div class="container">
    ...
    <div class="main">
        ...
        <div class="topbar" style="display:flex; align-items:center; justify-content:space-between;">
            ...
            <div style="width:38px; height:38px; border-radius:50%; background:#0B7C75; color:white; display:flex; align-items:center; justify-content:center; font-weight:bold; font-size:14px;">
        ...
        <div class="cards">
            ...
            <div class="card"><h3>Scheduled</h3><p><?= $scheduled ?></p></div>
        ...
        <div class="controls">
            <p style="font-size:11px; color:#888; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:10px;">Add appointment manually</p>
            <form method="POST">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            ...
                            <small style="color:#888;"><?= htmlspecialchars($row['age']) ?> · <?= htmlspecialchars($row['gender']) ?></small>
                        </td>
                        ...
                        <td>
                            <?php if($row['source'] === 'Patient Form'): ?>
                                <span style="background:#dbeafe; color:#1e40af; font-size:11px; padding:3px 9px; border-radius:5px;">Patient Form</span>
                            <?php else: ?>
                                <span style="background:#f3f4f6; color:#374151; font-size:11px; padding:3px 9px; border-radius:5px;">Manual</span>
                            <?php endif; ?>
                        </td>
