...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    $stmt->execute();
   header("Location: prescription.php?saved=1");
    exit;
}

// ─── LOAD APPOINTMENTS FOR DROPDOWN ───────────────────────────────────────────
$apts = $conn->query("SELECT id, patient_name, date, department 
                       FROM appointments 
...
<head>
    ...
    <style>
        .presc-card { background:white; border-radius:10px; padding:24px; box-shadow:0 2px 8px rgba(0,0,0,0.08); max-width:650px; }
        .presc-card label { display:block; font-size:13px; color:#555; margin-bottom:4px; margin-top:14px; font-weight:600; }
        .presc-card input, .presc-card textarea, .presc-card select { width:100%; padding:9px 12px; border:1px solid #ddd; border-radius:6px; font-size:14px; font-family:inherit; box-sizing:border-box; }
        .presc-card textarea { resize:vertical; }
        .btn-save { margin-top:18px; background:#0B7C75; color:white; border:none; padding:10px 24px; border-radius:6px; font-size:14px; cursor:pointer; }
        .btn-save:hover { background:#096b65; }
        .alert-success { background:#d1fae5; color:#065f46; padding:10px 16px; border-radius:6px; margin-bottom:16px; font-size:14px; }
        .badge-existing { background:#fef3c7; color:#92400e; font-size:12px; padding:3px 10px; border-radius:5px; margin-left:8px; }
        .apt-select-wrap { background:white; border-radius:10px; padding:20px 24px; box-shadow:0 2px 8px rgba(0,0,0,0.08); max-width:650px; margin-bottom:20px; }
        .apt-select-wrap label { font-size:13px; font-weight:600; color:#555; display:block; margin-bottom:6px; }
        .apt-select-wrap select { width:100%; padding:9px 12px; border:1px solid #ddd; border-radius:6px; font-size:14px; }
...
<div class="container">
    ...
    <div class="main">
        ...
        <div class="topbar">
            ...
            <p style="margin:0; font-size:13px; color:#888;">Select a patient then fill or update their prescription</p>
        </div>

        <?php if ($saved_msg): ?>
            <div style="background:#d1fae5; color:#065f46; padding:12px 18px; border-radius:8px; margin-bottom:16px; font-size:14px;">
        ...
        <div class="apt-select-wrap">
            ...
            <select id="aptDropdown" onchange="goToPatient(this.value)">
                ...
                ?>
                <option value="<?= $row['id'] ?>" <?= $selected_apt_id == $row['id'] ? 'selected' : '' ?>>
    ...
    <?php if ($apt_info['status'] === 'Cancelled'): ?>
        <div style="background:#fee2e2; color:#dc2626; padding:16px 20px; border-radius:8px; max-width:650px; font-size:14px;">
            This appointment was <strong>cancelled</strong>. Prescriptions cannot be added or edited for cancelled appointments.
        </div>
    <?php else: ?>
        <div class="presc-card">
            ...
            <div style="display:flex; align-items:center; margin-bottom:16px; padding-bottom:12px; border-bottom:1px solid #eee;">
                <div style="width:44px; height:44px; border-radius:50%; background:#0B7C75; color:white; display:flex; align-items:center; justify-content:center; font-weight:bold; font-size:15px; margin-right:12px;">
                    <?= strtoupper(substr($apt_info['patient_name'], 0, 2)) ?>
                </div>
                <div>
                    <div style="font-weight:bold; font-size:16px;"><?= htmlspecialchars($apt_info['patient_name']) ?></div>
            ...
            <form method="POST" action="prescription.php">
                ...
                <input type="hidden" name="apt_id" value="<?= $apt_info['id'] ?>">
                ...
                <label>
                    Medicine
                    <?php if ($existing): ?>
                        <span style="font-weight:400; color:#888;">(previous medicines shown — add new ones below)</span>
                    <?php endif; ?>
                </label>
                <textarea name="medicine" rows="5" placeholder="e.g. Paracetamol 500mg&#10;Amoxicillin 250mg" required><?= htmlspecialchars($existing['medicine'] ?? '') ?></textarea>
                ...
                <textarea name="notes" rows="3" placeholder="Any extra instructions, follow-up date, etc."><?= htmlspecialchars($existing['notes'] ?? '') ?></textarea>
